...
export function auctionTermsPack(
  ...
  const termsText = resolvedAuctionTerms(event);
  ...
  return {
    ...
    intro: `Read the terms for ${auctionName} before you drop a paddle. The $${BID_PREAUTH_AMOUNT} pre-authorization is for this auction only — not a charge until you pay a winning hammer.`,
    termsText,
    sections: sections.length
      ? sections
      : [{ heading: `${auctionNumber} terms`, paragraphs: [termsText] }],
    termsCheckbox: `I have read and agree to the Terms & Conditions for ${auctionNumber} (${auctionName}), ending ${endsLabel}.`,
